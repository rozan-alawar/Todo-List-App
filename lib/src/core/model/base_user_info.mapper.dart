...
class BaseUserInfoMapper extends ClassMapperBase<BaseUserInfo> {
  ...
  static BaseUserInfo _instantiate(DecodingData data) {
    return BaseUserInfo(email: data.dec(_f$email), phone: data.dec(_f$phone));
  }

  @override
  final Function instantiate = _instantiate;

  static BaseUserInfo fromJson(Map<String, dynamic> map) {
    return ensureInitialized().decodeMap<BaseUserInfo>(map);
  }

  static BaseUserInfo deserialize(String json) {
...
abstract class BaseUserInfoCopyWith<$R, $In extends BaseUserInfo, $Out>
  ...
  $R call({String? email, String? phone});
...
class _BaseUserInfoCopyWithImpl<$R, $Out>
  ...
  @override
  $R call({String? email, Object? phone = $none}) => $apply(FieldCopyWithData(
      {if (email != null) #email: email, if (phone != $none) #phone: phone}));
  @override
